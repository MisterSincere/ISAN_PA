...
The implementation and design closely followed discussed requirements and guidelines.
This chapter recalls some of these procedures for contrast ratio of texts, discusses their results and investigates further guidelines of the WCAG 2.1 for target area sizes.
...
Although no API has been implemented that delivers weather warnings, since those are mostly paid services, the layout is prepared for easy future extensions; thanks to the component system only the according `Weather.vue` component needs to be changed for that.
...
Both the top bar and the sidebar remain fixed in position so users always have a consistent reference point, regardless of which module they open.
...
The initial dropzones only allow for half-splits because adding options to place quads would double the amount of dropzones to eight and make interacting with them more difficult.
...
Thus they have been kept to make closing multiple tiles at once faster.
Additionally a dropzone has been added at the bottom for redundancy, where tiles can be dragged onto to remove it from the dashboard.
